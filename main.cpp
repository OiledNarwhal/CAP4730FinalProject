...
static float ambientLight[] = { .5, .5, .5, .001 };
static float diffuseLight[] = { .5, .5, .5, .001 };
static float specularLight[] = { .1, .1, .1, .001 };
float lightPosition[] = { 1000.0f, 1500.0f, 1000.0f, .005f }; // was 10.0f, 15.0f, 10.0f, 1.0f
...
void CreateYourOwnMesh()
{
	...
	for (int i = 0; i < 1025; i++)
	{
		...
		{
			std::cout << "Moist Map: " << terrainMesh.moistMap[i][j] << "\n";
		}
	}
	*/

	gTriangleMeshes[0]->moistureForColor = terrainMesh.moistMap;
	...
	for (int i = 0; i < 1025; i++)
	{
		for (int j = 0; j < 1025; j++)
		{
			std::cout << "Moisture For Color: " << gTriangleMeshes[0]->moistureForColor[i][j] << "\n";
		}
	}
	*/
}
...
void DisplayCallback()
{
	...
	if (meshType == MeshType::Mesh)
	{
		...
		*/
		...
		for (int id = 0; id < (int)gTriangleMeshes.size(); id++) {
			gTriangleMeshes[id]->moistureForColor = terrainMesh.moistMap;
			gTriangleMeshes[id]->numVerts = terrainMesh.numVerts;
