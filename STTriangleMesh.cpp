	...
	*/
    ...
    for (unsigned int i = 0; i < mFaces.size(); i++) {
        ...
        for (unsigned int j = 0; j < 3; j++) {
            ...
            glTexCoord2f(f->texPos[j]->x, f->texPos[j]->y); //This was commented out by me.
			...
			if (f->v[j]->pt.y <= 22)
				//glColor3f(11.0f/255, 96.0f/255, 5.0f/255);
				glColor3f(17.0f / 255, 48.0f / 255, 132.0f / 255);
			else if(f->v[j]->pt.y > 22 && f->v[j]->pt.y <= 23.5)
				glColor3f(226.0f/ 255, 204.0f/ 255, 115.0f/ 255);
			else if(f->v[j]->pt.y <= 29)
				glColor3f(15.0f / 255.0, 53.0f / 255.0, 4.0f / 255.0);
			else
				glColor3f(35.0f / 255.0, 30.0f / 255.0, 2.0f / 255.0);
