...
void PerlinMap::buildHeightMap() {
	...
	for (int i = 0; i < width; i++) {
		for (int j = 0; j < width; j++) {
			...
			float x = convertVal(sampleVal1) + 0.5 * convertVal(sampleVal2) + 0.25 * convertVal(sampleVal3) + 0.13 * convertVal(sampleVal4) + 0.06 * convertVal(sampleVal5) + 0.03 * convertVal(sampleVal6);
			x = x / (1.0 + 0.5 + 0.25 + 0.13 + 0.06 + 0.03);
			//std::cout << "Height: " << pow(x, 3) << "\n";
			//std::cout << sampleVal << std::endl;
			heightMap[i][j] = x; // was heightMap[i][j] = pow(x, 3);
		}
	}
