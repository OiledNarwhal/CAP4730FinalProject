...
void MySphere::InitVertices(void)
{
	...
	for (int i = 0; i < numVerts; i++)
	{
		for (int j = 0; j < numVerts; j++)
		{
			...
			//Was: m_vertices.push_back(STVector3(float(i) * float(0.5/numVerts), 0, float(j) * float(0.5/numVerts))); //Was i * 0.2, 0, j * 0.2
		}
	}
...
void MySphere::Create(int levels)
{
	...
	if (option == TerrainType::DS) {
		...
		PerlinMap arrayMan = PerlinMap(257);
	...
	for (int i = 0; i < numVerts; i++)
	{
		for (int j = 0; j < numVerts; j++)
		{
			...
			float tempMoisture = moistArray[i][j];
			m_vertices.at((numVerts * i) + j).y = temp;
			moistMap[i][j] = tempMoisture;
		}
	}
	...
	//If we are out of out bounds.
		...
		for (int i = 0; i < numVerts; i++)
		{
			for (int j = 0; j < numVerts; j++)
			{
				...
				float tempMoisture = moistArray[i + offsetX][j + offsetY];
